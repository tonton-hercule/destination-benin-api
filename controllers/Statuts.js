const StatutsModel = require("../models/Statuts")

const dataController = {
    /**Define all function of this controller */
    findAll: async (req, res) => {
        await StatutsModel.find().then(data => {
            res.status(200).send({ statuts: data })
        }).catch(err => {
            res.status(500).json({
                message:
    ...
    findOne: async (req, res) => {
        await StatutsModel.findById(req.params.id).then(data => {
            res.status(200).send({ statut: data })
        }).catch(err => {
            res.status(500).send({
                message: err.message
            })
    ...
    update: async (req, res) => {
        ...
        await StatutsModel.findByIdAndUpdate(req.params.id, req.body, { useFindAndModify: false }).then(data => {
            if (!data) {
                res.status(404).send({
    ...
    destroy: async (req, res) => {
        await StatutsModel.findByIdAndRemove(req.params.id, res).then(data => {
            if (!data) {
                res.status(404).send({
                    message: `Aucun élément correspondant`
                });
